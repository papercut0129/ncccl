...
入口实现在 [collectives.cc](src/collectives.cc#L22)。这些函数只负责异步入队，真正通信在 CUDA stream 上完成。

### 0.2 Device 侧统一分发

设备端主循环是 [ncclKernelMain](src/device/common.h#L434)：
...
每个集合算子都调用 [ncclCollCbdPart](src/include/device.h#L357)，得到：
...
见 [primitives.h](src/device/primitives.h#L37)。
...
声明见 [nccl.h.in](src/nccl.h.in#L559)。

### 1.3 Host 入队

[ncclBroadcastConfigImpl](src/collectives.cc#L271) 构造：
...
在本仓库当前源码中，Broadcast 设备内核只生成了 Ring 算法，见 [broadcast.h](src/device/broadcast.h#L23)。这限定的是当前这份 check-out 的设备端内核矩阵；经典 NCCL 版本中的 Broadcast 也存在 Tree 实现，但本仓库 [generate.py](src/device/generate.py#L104) 为 Broadcast 只保留了 `["RING"]`。
...
[broadcast.h](src/device/broadcast.h#L85) 提供三种特化：
...
当 `isNetOffload == true` 时，只用一个 warp 驱动网络通信，其余 warp 负责 root 非原地情况下的本地拷贝。实现见 [broadcast.h](src/device/broadcast.h#L30)。
...
声明见 [nccl.h.in](src/nccl.h.in#L530)。

### 2.3 Host 入队

[ncclReduceConfigImpl](src/collectives.cc#L354) 构造 `ncclInfo`，其中 `op` 是真实归约操作，`root` 是结果落点 rank。

### 2.4 设备端算法：Ring

在本仓库当前源码中，Reduce 设备内核也只生成了 Ring 算法，见 [reduce.h](src/device/reduce.h#L23)。这里同样只针对当前 check-out：经典 NCCL 版本中的 Reduce 可以有 Tree 实现，但本仓库 [generate.py](src/device/generate.py#L104) 为 Reduce 只保留了 `["RING"]`。
...
[reduce.h](src/device/reduce.h#L72) 提供 SIMPLE、LL、LL128 三个特化。
...
声明见 [nccl.h.in](src/nccl.h.in#L606)。

### 3.3 Host 入队

[ncclAllGatherConfigImpl](src/collectives.cc#L153) 构造 `ncclInfo`，不归约，`op` 用 `ncclSum` 占位。

### 3.4 设备端算法

AllGather 支持四种算法，见 [all_gather.h](src/device/all_gather.h#L20)：
...
实现见 [all_gather.h](src/device/all_gather.h#L20)。

Ring 的三种协议特化见 [all_gather.h](src/device/all_gather.h#L102)。

#### 3.4.2 PAT AllGather

PAT 是 Parallel Aggregated Tree。设备端用专门的“算法计算线程”生成 `ncclPatStep`，worker 线程按步骤搬运。

实现见 [all_gather.h](src/device/all_gather.h#L131)。

#### 3.4.3 NVLS AllGather

利用 NVSwitch 的 scatter/gather 能力，将数据按 rail 切分并完成汇聚。

实现见 [all_gather.h](src/device/all_gather.h#L290)。
...
实现见 [all_gather.h](src/device/all_gather.h#L520)。
...
声明见 [nccl.h.in](src/nccl.h.in#L589)。

### 4.3 Host 入队

[ncclReduceScatterConfigImpl](src/collectives.cc#L393) 构造 `ncclInfo`，`count` 字段填的是 `recvcount`。

注意 `ncclFuncSendCount` 对 ReduceScatter 会返回 `nRanks * count`，用于计算实际发送缓冲大小。

### 4.4 设备端算法

ReduceScatter 支持四种算法，见 [reduce_scatter.h](src/device/reduce_scatter.h#L20)：
...
实现见 [reduce_scatter.h](src/device/reduce_scatter.h#L460)。
...
声明见 [nccl.h.in](src/nccl.h.in#L575)。

### 5.3 Host 入队

[ncclAllReduceConfigImpl](src/collectives.cc#L234) 构造 `ncclInfo`。AllReduce 是算法空间最大的集合算子，调优阶段会根据消息大小、拓扑、硬件选择算法和协议。

### 5.4 设备端算法

AllReduce 支持六种算法，见 [all_reduce.h](src/device/all_reduce.h#L25)：
...
实现见 [all_reduce.h](src/device/all_reduce.h#L25)。

Ring 的三个协议特化见 [all_reduce.h](src/device/all_reduce.h#L296) 和文件末尾的 LL/LL128 特化。
...
实现见 [all_reduce.h](src/device/all_reduce.h#L120) 和 [all_reduce.h](src/device/all_reduce.h#L193)。

Tree + SIMPLE 的入口见 [all_reduce.h](src/device/all_reduce.h#L306)，Tree + LL/LL128 见文件末尾。

#### 5.4.3 CollNet Direct / CollNet Chain

- CollNet Direct：借助集合网络硬件，按 Scatter / Gather / Reduce / Broadcast 四段流水完成，见 [all_reduce.h](src/device/all_reduce.h#L319)。
- CollNet Chain：链式上行归约、下行广播，见 [all_reduce.h](src/device/all_reduce.h#L712)。

二者都依赖集合网络，如 InfiniBand SHARP。

#### 5.4.4 NVLS 与 NVLS_TREE

- NVLS：使用 NVSwitch 的硬件归约能力完成 scatter/gather/reduce/broadcast，见 [all_reduce.h](src/device/all_reduce.h#L462)。
- NVLS_TREE：NVLS 负责归约，tree 负责多节点分发/汇聚，见 [all_reduce.h](src/device/all_reduce.h#L600)。
...
[collectives.cc](src/collectives.cc#L469) 中：
...
设备端统一使用 [sendrecv.h](src/device/sendrecv.h#L18) 的：
...
公开 API 见 [collectives.cc](src/collectives.cc#L197)、[collectives.cc](src/collectives.cc#L317)、[collectives.cc](src/collectives.cc#L431)。
...
公开 API 见 [collectives.cc](src/collectives.cc#L521)、[collectives.cc](src/collectives.cc#L554)、[collectives.cc](src/collectives.cc#L584)。

### 8.2 实现方式：RMA 队列

这些算子被 `ncclTaskClassification` 归入 `rmaTaskQueue`，不进入传统集合内核调度，见 [task_classify.cc](src/enqueue/task_prep/task_classify.cc#L130)。
...
设备端见 [all_gather_v.h](src/device/all_gather_v.h#L16)：
...
公开归约操作符见 [nccl.h.in](src/nccl.h.in#L448)：

```c
ncclSum, ncclProd, ncclMax, ncclMin, ncclAvg
```

设备端归约操作符见 [device.h](src/include/device.h#L60)：

```c
ncclDevSum, ncclDevProd, ncclDevMinMax, ncclDevPreMulSum, ncclDevSumPostDiv
```

映射关系在 [enqueue.cc](src/enqueue/enqueue.cc#L2480)：
