...
- 内部算子枚举：[nccl_common.h](src/include/nccl_common.h#L72)
- 对外 API 声明：[nccl.h.in](src/nccl.h.in#L517)
- 对外 API 入队实现：[collectives.cc](src/collectives.cc#L22)
- 设备端公共执行框架：[common.h](src/device/common.h#L434)
- 底层传输原语：[primitives.h](src/device/primitives.h#L26)
- 内核生成算法矩阵：[generate.py](src/device/generate.py#L20)
...
3. **组合型集合算子**：AlltoAll、Gather、Scatter。
   ...
   - 拆分逻辑见 [task_classify.cc](src/enqueue/task_prep/task_classify.cc#L58)。

4. **单边 / 信号算子**：PutSignal、Signal、WaitSignal。
   - 对应 `ncclPutSignal`、`ncclSignal`、`ncclWaitSignal`，走 RMA 任务队列，而不是传统集合内核。

此外还有内部优化算子 `AllGatherV`，它是变长 AllGather / 批量化 Broadcast 的内部实现，不是公开 API，设备端见 [all_gather_v.h](src/device/all_gather_v.h#L16)。
...
- API 注释见 [nccl.h.in](src/nccl.h.in#L517)。
- 入队包装见 [collectives.cc](src/collectives.cc#L153)。
...
例如 [collectives.cc](src/collectives.cc#L234) 中的 `ncclAllReduceConfigImpl` 只是把参数填进 `ncclInfo` 后交给统一入口。所有集合算子都是异步入队，函数返回只代表“已经提交到 CUDA stream”，不代表通信完成。

### 3.2 Device 侧统一分发

设备端主循环在 [common.h](src/device/common.h#L434) 的 `ncclKernelMain`：
...
所有集合算子都使用 channel 并行，并由 [ncclCollCbdPart](src/include/device.h#L357) 计算：
...
协议定义在 [primitives.h](src/device/primitives.h#L37)：
...
这是所有归约算子共有的数值语义基础，详见 [reduce_kernel.h](src/device/reduce_kernel.h#L204)。
...
它们必须携带真实的 `ncclRedOp_t op`，并经过 [enqueue.cc](src/enqueue/enqueue.cc#L2480) 把公开 `ncclRedOp_t` 映射成设备端 `ncclDevRedOp_t`：
...
算法支持由 [generate.py](src/device/generate.py#L74) 中的 `algos_of_coll` 决定：
...
- 上表反映的是**本仓库当前这份源码的设备端内核矩阵**：Broadcast 和 Reduce 只生成了 Ring 内核，没有独立的 Tree 内核。
- 这不等同于“NCCL 官方所有版本都只有 AllReduce 支持 Tree”。经典 NCCL 版本及不少资料中，Tree 也用于 Broadcast / Reduce；本仓库则是通过 [generate.py](src/device/generate.py#L104) 的 `algos_of_coll` 把这两个算子的算法列表固定为 `["RING"]`，并在 [tree.cc](src/tuning/tree.cc#L22) 中对非 AllReduce 的 Tree 模型做了硬禁用。
...
协议与算法组合由 [generate.py](src/device/generate.py#L83) 的 `required_cuda` 约束：

- 在本仓库当前源码中，RING 支持 LL、LL128、SIMPLE 三种协议；TREE 只存在于 AllReduce，且同样支持 LL、LL128、SIMPLE。
...
见 [task_classify.cc](src/enqueue/task_prep/task_classify.cc#L58)。
...
它们在 `ncclTaskClassification` 中被归入 `rmaTaskQueue`，见 [task_classify.cc](src/enqueue/task_prep/task_classify.cc#L130)。
